...
- `programs/anchor` - Anchor Rust workspace containing the Harvverse program.
- `packages/solana-client` - Shared Solana helpers plus the Codama-generated Harvverse client under `src/generated/harvverse`.
...
`pnpm run setup` requires local Anchor CLI and Solana CLI installs. It builds `programs/anchor` and regenerates `packages/solana-client/src/generated/harvverse`.
...
Use `pnpm dev:local` for a full local validator workflow. It deploys the canonical program ID locally and initializes Harvverse `ProgramConfig` if missing.

Use `pnpm harvverse:config:devnet` to dry-run devnet `ProgramConfig` initialization. Add `--send` only when you intend to submit the devnet transaction.
