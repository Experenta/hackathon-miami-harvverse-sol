...
export default function SettlementPreviewScreen() {
  ...
  return (
    <Screen scrollable contentContainerStyle={{ gap: theme.spacing.xl }}>
      ...
      <Section
        description="This remains a preview using the existing demo settlement constants."
        aside={<Badge label="Projected" tone="warning" />}
      >
        <View
          style={{
            flexDirection: "row",
      ...
      <Section
        ...
        description="Mathematics are unchanged; this is a clearer statement format."
      >
        <Card variant="success">
          <DetailRow
            label="Revenue formula"
            value={`${DEMO_YIELD_QQ}qq x ${LBS_PER_QQ} lb/qq x $${PRICE_PER_LB.toFixed(2)}`}
            valueTone="secondary"
          />
          <DetailRow label="Revenue" value={formatUsd(revenueCents)} />
          <DetailRow label="Cost" value={formatUsd(costCents)} />
          <DetailRow label="Profit" value={formatUsd(profitCents)} />
          <DetailRow
