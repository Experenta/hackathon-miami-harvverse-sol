...
New directory at the workspace root. Convex is not currently installed — setup requires `npx convex init` and configuration.
...
├── http.ts                       # HTTP actions (health endpoint)
└── tsconfig.json
```
