...
## This Project: Convex is Already Set Up

In this monorepo, Convex is **already installed and configured** at `packages/backend/`. You do not need to scaffold a new project.

**Always read `packages/backend/convex/_generated/ai/guidelines.md` first** before writing any Convex code.

### What's already in place

- `packages/backend/` — the `@havverse/backend` workspace package
- `packages/backend/convex/schema.ts` — the Convex schema (currently minimal, needs Harvverse tables)
- `packages/backend/convex/_generated/` — auto-generated types (committed to git)
- `apps/native/components/convex/convex-client-provider.tsx` — `ConvexProvider` already wired in the native app
- `apps/native/package.json` — already has `"@havverse/backend": "workspace:*"` and `"convex": "^1.37.0"`

### Environment variables

| Location                      | Variable                 | Purpose                     |
| ----------------------------- | ------------------------ | --------------------------- |
| `packages/backend/.env.local` | `CONVEX_DEPLOYMENT`      | Convex deployment slug      |
| `packages/backend/.env.local` | `CONVEX_URL`             | Convex backend URL          |
| `apps/native/.env.local`      | `EXPO_PUBLIC_CONVEX_URL` | URL exposed to the Expo app |

Copy `packages/backend/.env.example` → `packages/backend/.env.local` and fill in values after running `npx convex dev` inside `packages/backend/`.

### Commands (from repo root)

```bash
pnpm dev:convex          # Start Convex dev watcher (packages/backend)
pnpm convex:setup        # Deploy schema + functions (convex dev --until-success)
pnpm convex:codegen      # Regenerate types only
```

Or from inside `packages/backend/`:

```bash
pnpm dev                 # npx convex dev (long-running watcher)
pnpm setup               # npx convex dev --until-success
pnpm codegen             # npx convex codegen
```

### Adding schema tables and functions

All Convex files go inside `packages/backend/convex/`. The file-based routing maps directly:

- `packages/backend/convex/users.ts` → `api.users.*`
- `packages/backend/convex/lots.ts` → `api.lots.*`
- etc.

Import the generated API in the native app:
...
## General Quickstart Reference (for new projects)

The sections below apply when starting a brand-new Convex project outside this monorepo.
...
Ask the user to run `npx convex dev` themselves — it requires browser-based OAuth on first run and is a long-running watcher that agents should not start directly.
...
- [ ] `packages/backend/.env.local` has `CONVEX_DEPLOYMENT` and `CONVEX_URL`
- [ ] `apps/native/.env.local` has `EXPO_PUBLIC_CONVEX_URL`
- [ ] `packages/backend/convex/_generated/` exists with `api.ts` and `server.ts`
- [ ] `pnpm dev:convex` runs without errors
