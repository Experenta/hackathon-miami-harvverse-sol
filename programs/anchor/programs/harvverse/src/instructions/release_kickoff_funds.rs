...
const KICKOFF_RELEASE_INDEX: u8 = 0;

#[derive(Accounts)]
pub struct ReleaseKickoffFunds<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<ReleaseKickoffFunds>) -> Result<()> {
    release_funds(ctx, KICKOFF_RELEASE_INDEX)
}

fn release_funds(ctx: Context<ReleaseKickoffFunds>, release_index: u8) -> Result<()> {
